...
$(function () {
    ...
    mip.groupId = App.getUrlParam("groupId");
	...
	setTimeout(function () {
		...
		if(fm_strategy == "0"){//成本
			...
			mip.hMipAmt = Number(fm_addAmt) > 0 ? fm_addAmt : 0;
			mip.icIndex = fm_addRate;
			mip.lMipAmt = Number(fm_lowAmt) > 0 ? fm_lowAmt : 0;
			...
			mip.lMipAmt = high.replace(/,/g,'')< minMipAmt ? App.formatMoney(minMipAmt,2).replace(/,/g,'') : high.replace(/,/g,'');// 加码金额
			mip.dcrIndex = 0.6;// 减码指标码
			mip.hMipAmt = low.replace(/,/g,'') < minMipAmt ? App.formatMoney(minMipAmt,2).replace(/,/g,'') : low.replace(/,/g,'');// 减码金额
...
$(".mipbuyamt_zh").on("input", function () {//定投金额
	checkInput()
});
//检查输入框 更新按钮状态
function checkInput(){
...
function setCard(card){
	...
	mip.bankDisplayCard = card.bankAccoDisplay;
	mip.bankName = card.bankName;
    ...
    App.get("/mobile-bff/v1/fund-group/detailInfo?groupId=" + groupId, null, function(result){
        ...
        if(result.body != undefined && result.body != null){
            ...
            if(detailInfo != undefined && detailInfo != ""){
                ...
                $(".fund-name").html(mip.groupName);
                if(App.isEmpty(detailInfo.initLimitAmount)){
                    minMipAmt = 0;
					$(".mipbuyamt_zh").attr("placeholder", "0.00元起");
                    
                } else {
                    minMipAmt = Number(detailInfo.initLimitAmount);
                    $(".mipbuyamt_zh").attr("placeholder", detailInfo.initLimitAmount + "元起");
                }
				if(detailInfo.supportTargetProfit == "1"){
					if(App.getUrlParam("fm_mipRate") && Number(App.getUrlParam("fm_mipRate")) == 0){//特殊处理 没有入参就不展示20210322
						$(".mbsy").hide();	
					}else{
						if(arAcct || contractno){
							$(".mbsy").hide();
						}else{
							$(".mbsy").show();
						}
				...
				var html0 = '<p class="rules" style="font-size:0.635rem;margin-left: .75rem;"><span id="isRead" class="chose-icon "></span>我已同意<a href="http://static.99fund.com/mobile/agreement/funds_group_agreement.html">《汇添富组合产品服务协议》</a>&nbsp;</p>'+
				'<p class="rules" style="font-size:0.635rem;margin-left: .75rem;">'+
				'<span id="isRead0" class="chose-icon "></span>我已阅读并了解<a href="mip_contract.html?groupId='+groupId+'">各基金产品资料概要</a>&nbsp;</p>';
		...
		App.get(url, null, function (result){
			if(result.body!= undefined && result.body != null && result.returnCode == 0){
				...
				if(con_cashFrm == 'V'){
					...
					mip.bankDisplayCard = '尾号'+ con_bankAccoDisplay;
					mip.bankName = con_bankName;
				}
...
function mipIntelligent() {
    ...
    var purchaseObj = {};
	purchaseObj.mipsetup = mip.mipsetup;
	purchaseObj.mipDesc = mip.mipDesc;
	...
	purchaseObj.isSupportStrategy = mip.isSupportStrategy;
	purchaseObj.isOrderImmediately = mip.isOrderImmediately;
	purchaseObj.isOpenTargetProfit = mip.isOpenTargetProfit;
	purchaseObj.mipKind = mip.isSupportStrategy;
	purchaseObj.dcrIndex = mip.dcrIndex;
	purchaseObj.hMipAmt = mip.hMipAmt;
	purchaseObj.icIndex = mip.icIndex;
	purchaseObj.lMipAmt = mip.lMipAmt;
	purchaseObj.referIndexCode = mip.referIndexCode;
	purchaseObj.referAverageCode = mip.referAverageCode;
	purchaseObj.amt = mip.mipbuyamt;
	purchaseObj.targetProfit = mip.targetProfit;
	purchaseObj.isContinueHold = mip.isContinueHold;

	purchaseObj.autoRenewal = mip.autoRenewal;
	...
	if(contractno){
		window.location.href = "confirm.html?is_strategy=1&contractno="+ contractno;
	}		
    else{
		window.location.href = "confirm.html?is_strategy=1";
	}
	...
	$('.setting_target_rate_on_off2').on('click',function(){
		...
		if (setting_target_rate2 == "0") {
			...
			if(App.numberFormat($(".mipbuyamt_zh").val()) <= 0){
				...
				$("#below_deduction_amt").val(low.replace(/,/g,'')< minMipAmt ? App.formatMoney(minMipAmt,2).replace(/,/g,'') : low.replace(/,/g,''));
		    	$("#above_deduction_amt").val(high.replace(/,/g,'')< minMipAmt ? App.formatMoney(minMipAmt,2).replace(/,/g,'') : high.replace(/,/g,''));
			}
		} else {
	...
	$(".setProfit2").click(function(){
		...
		if(App.numberFormat($(".mipbuyamt_zh").val()) <= 0){
			...
			if($("#strategy_txt").html() == "均线策略"){
				...
				high = App.formatMoney($(".mipbuyamt_zh").val()*highYield,2);
				low = App.formatMoney($(".mipbuyamt_zh").val()*lowYield,2);
		    	$("#below_deduction_amt").val($("#below_deduction_amt").val()< minMipAmt ? App.formatMoney(minMipAmt,2).replace(/,/g,'') : $("#below_deduction_amt").val());
		    	$("#above_deduction_amt").val($("#above_deduction_amt").val()< minMipAmt ? App.formatMoney(minMipAmt,2).replace(/,/g,'') : $("#above_deduction_amt").val());
				mip.mipsetup = "0";
			}
			$("#down_dinqi").show();
		}
...
if(domain.indexOf("sit") >= 0 || domain.indexOf("uat") >= 0 || domain.indexOf("mdev") >= 0 || domain.indexOf("local") >= 0){//uat
	//测试2种
