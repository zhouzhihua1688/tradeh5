            ...
            if(fundGDetails != null && fundGDetails != undefined){
                for(var i=0; i<fundGDetails.length; i++){
                    ...
                    var listHtml = "<tr onclick='forwardFundDetailPage(\"" + fundGInfo.fundId + "\")' " + ((i >= 3) ? " style='display: none' class='open' " : "") + "  >"
                        ...
                        + (fundGInfo.fundTradeStName == null ? "" : "<i class='deal'>" + fundGInfo.fundTradeStName + "</i>")
                        +"</td>"
...
function queryCharts(queryTp){
    var url = "/mobile-bff/v1/fund-group/fundgroup-yields?groupId="+ App.getUrlParam("groupId") +"&queryTp="+ queryTp +"&chartTp=1,3,4&createDt="+ fundGroupCreateDate;
    App.get(url, null, function(resultStr){
        var data = typeof resultStr === 'string' ? JSON.parse(resultStr) : resultStr;
        if(data.body != undefined && data.body != null){
