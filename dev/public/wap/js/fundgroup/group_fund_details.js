...
var option_1 = {
    ...
    tooltip : { show: true, trigger: 'axis',
        ...
        formatter: function(params){
            var arr = [];
arr.push('<div style="width: 100%;margin-left: -82%;font-size: 14px;">20'+params[0].axisValue.replace(/-/g,'.') + '  ')
                
            arr.push(params[0].seriesName + ": " +params[0].data + "%");
            arr.push("</div>");
            return arr.join("");
        }
...
var option_2 =  {
    ...
    tooltip : { show: true, trigger: 'axis',
        ...
        formatter: function(params){
            var arr = [];
arr.push('<div style="width: 100%;margin-left: -80%;font-size: 14px;">20'+params[0].axisValue.replace(/-/g,'.'))
...
function requestGroupFundDetail(groupId){
        ...
        if(result.body != undefined && result.body != null){
            ...
            if(detailInfo != undefined && detailInfo != ""){
				...
				$("#fundgroupDesc").html('<div class="tupian"><img src="../images/fundgroup/jianyi.png" alt=""></div><span style="font-weight:bold">推荐建议：</span>'+detailInfo.fundgroupDesc);
				...
				if(detailInfo.fundGroupType == "12"){
					var newStr = detailInfo.shiftDetailRes.bidEndDate;
					$("#createtimeStr").html("最近发车日："+newStr.substr(0,4)+"年"+newStr.substr(-4,2)+"月"+newStr.substr(-2,2)+"日");
				}else{
                ...
                } else if (detailInfo.halfYearProfit == null || detailInfo.halfYearProfit == '--'){
                    $("#profit_six").html("--");
                }
                ...
                } else if (detailInfo.nearYearProfit == null || detailInfo.nearYearProfit == '--'){
                    $("#profit_year").html("--");
                }
				...
				if(App.isNotEmpty(detailInfo.changeAdvise)){
                ...
                for(var i in groupDetails){
                    ...
                    for(var j in subFunds){
                        ...
                        if (fundRiskLevel != undefined && fundRiskLevel != null) {
                            ...
                            fundRiskDesc = fund.fundRiskLevelNm;
                        }
				...
				if(fundGroupType == "12"){
					...
					// $(".btn_mip").hide();
                    $(".btn_mip").show();
                    $(".btn_mip").off().css('background-color', '#999').children('a').off().css('color', '#fff');
					...
					$("#tabs").show();
					$(".btn_mip").show();
                    // S 20211107 根据canAutoInvested字段确定是否展示 定投
                    if(detailInfo.canAutoInvested == "N"){
                        $(".btn_mip").off().css('background-color', '#999').children('a').off().css('color', '#fff');
                    }
                    // E 20211107 根据canAutoInvested字段确定是否展示 定投
                    
				}
                getChartData(groupId, "0,1,4", 2, establishDateStr);
                if(detailInfo.canPurchase == "Y"){
                    $(".buy_btn_li").show();
					$(".buy_btn_li a").click(function(){
						buttonJump('BUY');
					});
                }else{
					if(detailInfo.buttonTips){
						$(".buy").html(detailInfo.buttonTips);
						$(".buy_btn_li").css({'background':'#d4c2c2'}).show();						
					} else {
                        $(".buy_btn_li").show().css('background-color', '#999').children('a').off().css('color', '#fff');
                    }
				}
    ...
    function getNewChartData(groupId, chartTp, queryTp, createDt){
        //console.log(chartTp);
        $.get("/mobile-bff/v1/fund-group/fundgroup-yields?groupId=" + groupId + "&chartTp=" + chartTp + "&queryTp=" + queryTp + "&createDt=" + createDt + "&t=" + new Date().getTime(), function(resultStr){
        ...
        if(result.body != undefined && result.body != null){
            ...
            if(dateList != undefined && dateList != null){
                ...
                if(dataSize <= 0){
                    ...
                    $("#transferCount_a").attr("href", "fund_group_trans_his.html?groupId="+ App.getUrlParam("groupId") + "&date=" + lastChangeDateStr.replace(/-/g,''));
                }
...
function getChartData(groupId, chartTp, queryTp, createDt){
    ...
    App.get("/mobile-bff/v1/fund-group/fundgroup-yields?groupId=" + groupId + "&chartTp=" + chartTp + "&queryTp=" + queryTp + "&createDt=" + createDt + "&t=" + new Date().getTime(), null, function(resultStr){
        var result = typeof resultStr === 'string' ? JSON.parse(resultStr) : resultStr;
        if(result.body != undefined && result.body != null){
	...
	App.get(url, null, function(result){
			if(result.body != undefined && result.body != null){
				...
				result.body[App.getUrlParam("groupId")].forEach(function(item){
                    ...
                    var url='';
                    if(item.url){
                        url=item.url.replace('index','share');
                    }
					html += '<li class="jingzhi div-jump list" style=" margin-top: 0.01rem;" onclick="visit(\''+url+'\')">'
							+'	<p><img src="../images/fundgroup/sound.png" alt="" style="margin-right: .5rem;"><span >'+infoTitle+'</span></p>'
