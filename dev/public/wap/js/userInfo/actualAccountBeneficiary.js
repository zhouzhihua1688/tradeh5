...
function queryRealCustomerInfo() {
    var url = App.projectNm + "/customer_info/query_real_customer_info?handleTp=0&r=t" + (Math.random() * 10000).toFixed(0);
    App.get(url, null, function(result){
        if (result.body != undefined && result.body != null){
            var realInfo = result.body.realInfo;
            if(realInfo != undefined &&　realInfo != null){
                $(".isBeneficiary li").removeClass("current");
                if(realInfo.isBeneficiary == 'N'){
                    $(".isBeneficiary li").eq(1).addClass("current");
                    $(".content_2").show();
                    ...
                    $(".isBeneficiary li").eq(0).addClass("current");
                }
            }
...
function saveRealCustomerInfo() {
    var data = {"handleTp":"0"};
    var index = $(".isBeneficiary").find(".current").index();
    if(index == 0){
        data.isBeneficiary = "Y";
    } else {
