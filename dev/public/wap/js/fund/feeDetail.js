$(function() {
    ...
    if(queryType == 'T'){
        ...
        $("title").html('转购费率');
        $(".name").html('估算转购费用');
        $(".content3 h2").html('认/申购费率');
        $(".content3 ul li").eq(0).hide();      
        $(".content3 ul li").eq(2).hide();      
        $(".content3 .table-footer .newText").html('估算认/申购费用');      
        $(".remark_1 ").html('转购费用=转出基金赎回费+转入基金认/申购费');    
        $("#isShow").hide();  
    }
    else {
        $("title").html('赎回费率');
...
function calculateFee(serialNo,queryType,quty,oProductId) {
    ...
    // S 实盘组合内基金费率需调用fundgroup接口 20210525
    // productType: 1 实盘基金；0 普通基金
    var productType = App.getUrlParam('productType');
    productType == "1" && (url = '/fundgroup/v1/trades/fee/detail');
    // E 实盘组合内基金费率需调用fundgroup接口 20210525
    
    
    //queryType：T转换B购买R赎回A转购
    ...
    App.post(url, JSON.stringify(data), null, function (result) {
        ...
        if(info != undefined && info != null){
            ...
            // 20220622，需求10340美元赎回页文案优化，添加美元单位
            info.currencyUnit && $("#currencyUnit").text(info.currencyUnit);
