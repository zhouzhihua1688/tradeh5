...
$(function () {
    ...
    $(".giftlist a").on("click",function(event){
		...
		// 	$("#prompt1").hide();
		// 	$(".content2").hide();
		// } else if($(this).children(".title").text() == '自动赎回'){
		// 	$("#prompt1").show();
		// 	$(".content2").show();
		// }
	})
...
var view = new Vue({
    ...
    mounted: function () {
        ...
        queryFinancialFundDetail(_this);
    },
    methods: {
...
function queryAccount() {
    ...
    utils.get(url, null, function (result) {
        if (result.body != undefined && result.body != null) {
            ...
            $(".remark_tip small").html(xjbRemark);
            queryTips();
...
function queryFinancialFundDetail()
{
    ...
    utils.post(url, data, function (result) {
        if (result.body != undefined && result.body != null) {
            ...
            $("#view_fundNmId").html("<span id= 'fundName'>"+fundName + " </span><span id='fundId'>"+fundId+"</span>"); 
            }   
    })
}
        ...
        if (result.body != undefined && result.body != null) {
            ...
            if (fundContractList != undefined && fundContractList != null && fundContractList.length > 0){
				...
				listKey.forEach(function (item0, index0) {
					if(item0 == "KFS"){
						html = '<p class="rules" style="font-size: 0.635rem;margin-left: 2rem;margin-right: 0.5rem;margin-top: 0.5rem;margin-bottom: 0.5rem;"><span id="agreement'+index0+'"><span id="isRead'+index0+'" class="chose-icon"></span>我已阅读并确认已知悉</span>';
					}else{
						html = '<p class="rules" style="font-size: 0.635rem;margin-left: 2rem;margin-right: 0.5rem;margin-top: 0.5rem;margin-bottom: 0.5rem;"><span id="agreement'+index0+'"><span id="isRead'+index0+'" class="chose-icon"></span>我已同意</span>';
					}
	                fundContractList.forEach(function (item, index) {
					...
					App.bind("#agreement"+index0,"tap",function(){
						if(!$("#isRead"+index0).hasClass("current")){
					        ...
					        if(Number($("#purchaseAmt").val().replace(/,/g,"")) > 0  && (contractLength == chkCnt)){
...
//     $("#purchaseAmt").val('2000') ;
// });
// $(".fiveThousand").click(function(){console.log($("#avaliable").html());
//     //$("#subQuty").val(App.formatMoney($("#avaliable").html().replace(/,/g, "")/2));
//     $("#purchaseAmt").val('5000') ;
// });
// $(".tenThousand").click(function(){console.log($("#avaliable").html());
//     //$("#subQuty").val(App.formatMoney($("#avaliable").html().replace(/,/g, "")/3));
//     $("#purchaseAmt").val('10000') ;
// });
	    ...
	    utils.get(url, null, function(result) {
	        ...
	        if (body != null && body != undefined) {
	        	if(App.isNotEmpty(body.tradeRules)){
		            ...
		            $(".remark_tip small").html($(".remark_tip small").html() + '&nbsp;<i class="icon-i other" style="touch-action: pan-y; user-select: none; -webkit-user-drag: none; -webkit-tap-highlight-color: rgba(0, 0, 0, 0);">i</i>');
