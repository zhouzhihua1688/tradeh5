var fundNameStr = App.getSession("fundNameStr");
if(fundNameStr){
	$(".fund-name").html( (fundNameStr.length > 14 ? fundNameStr.substr(0,14)+'...' :fundNameStr) );
}
$("#totalAmt").html(App.getUrlParam("totalAmt"));
$(".mipbuyamt_zh").val(App.getUrlParam("totalAmt"));
//将参数存入数组
var fundList = App.getUrlParam("fundList");

var fundArr = fundList.split(",");
//基金列表数据
var fundPayInfos = App.getSession("fundPayInfos");
...
$(function () {
    ...
    $(".fund-name,#goBack").click(function () {
		var curUrl = window.location.href;
		if(curUrl.indexOf('edit') == -1){
			window.location.href = curUrl.replace('batchMip','batchMipList') + '&edit=1';;
		}else{
			window.location.href = curUrl.replace('batchMip','batchMipList');
		}
    });
			...
			App.get(url, null, function(result){
				if (result.body != undefined && result.body != null){
					...
					for(var k in referIndexInfo){
						if(fm_index && fm_index == referIndexInfo[k].referIndexName){
							mip.referIndexCode = referIndexInfo[k].referIndexCode;//参考指数
						}
					}
					
					for(var m in referAverageInfo){
						if(fm_line && fm_line == referAverageInfo[m].referIndexName){
...
function showSmTip() {
    ...
    // var url = App.projectNm + "/common/check_union_risk_level" ;
	var url = "/mobile-bff/v1/common/check-union-risk-level";
...
$(".icon-point").click(function () {
	if($(this).attr("data") == 1){
	    ...
	    var txt = "系统将分别为您选择的基金建立定投计划";
	    $("#alert4").html(txt)
		$(".Bomb-box-ok").html("我知道了");
		$(".Bomb-box").show();
	}else{
	    var txt = "目标收益设置是在普通定投基础上新增止盈策略，当定投收益率达到您设置的目标收益率时，定投计划即可止盈赎回。";
	    $(".Bomb-box1 .Bomb-box-main .Bomb-box-content p").html(txt)
...
function queryCard(successFun) {
    ...
    var url = "/mobile-bff/v1/pay/pay-bank-list?currencyType=156&tradeType=00&tradeScene=11" ;
	App.get(url, null, function (result) {
...
function queryFundDetail() {
    ...
    var url = "/mobile-bff/v1/fund/batch-fund-info?fundIds="+fundList;

    App.get(url,null,function(result){
        if (result.body != undefined && result.body != null){
            var fundInfo = result.body.batchFundInfos;
			var html = "";
			var html0 = '<p class="rules" style="font-size:0.635rem;margin-left: .75rem;"><span class="chose-icon isRead"></span><em style="width: 15rem;display: block;    float: right;    padding-right: 1.2rem;">我已同意<em style="color:#3295e8" onclick="gotoUrl(\'http://static.99fund.com/mobile/agreement/zhihuidingtou_agreement.html\')">《汇添富基金管理股份有限公司定投业务协议》</em>&nbsp;</em></p>';

			fundInfo.forEach(function(item,index){
				...
				if (fundContractList != undefined && fundContractList != null && fundContractList.length > 0){
					...
					listKey.forEach(function (item0, index0) {
						var htm_ms = '';
						if(item0 == "KFS"){
							html+= '<p class="rules" style="font-size:0.635rem;margin-left: .75rem;"><span class="chose-icon isRead"></span><em style="width: 15rem;display: block;    float: right;    padding-right: 1.2rem;">我已阅读并确认已知悉';
						}else{
							html+= '<p class="rules" style="font-size:0.635rem;margin-left: .75rem;"><span class="chose-icon isRead"></span><em style="width: 15rem;display: block;    float: right;    padding-right: 1.2rem;">我已同意';
						}
...
function batchMip() {
	if(Number(mip.periodNumber) > 0 && (mip.periodNumber.indexOf('.') > -1 || mip.periodNumber.indexOf('-') > -1 )){
		...
		var url = '/mobile-bff/v1/fund/batch-fund-mip-create';
		mip.fundPayInfos = fundPayInfos;
		var data = mip;
		utils.post(url, JSON.stringify(data), null, function (result) {
			...
			if (result.body != undefined && result.body != null){
				...
				App.setSession(App.serialNo_forword_url, "/mobileEC/wap/fund/batch_fund_mip_successfully.html");
...
if(domain.indexOf("sit") >= 0 || domain.indexOf("uat") >= 0 || domain.indexOf("mdev") >= 0){//uat
	//测试2种
