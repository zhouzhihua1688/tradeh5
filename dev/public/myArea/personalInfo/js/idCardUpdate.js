$(function () {
    ...
    async function nextStep() {
        ...
        //是否自动保存地址
        var isAuto = 1;
        if ($(".checkBox").attr('src') === 'img/checkBox.png') {
            isAuto = 0;
        }
        ...
        $.ajax({
            url: `/mobile-bff/v1/account/upload-cust-cert-pic-no-ocr?idTp=` + idTp + `&isAuto=`+ isAuto,
            type: "POST",
                    ...
                    if (res.body) {
                        ...
                        utils.ajax({
                            ...
                            data: JSON.stringify({
                                ...
                                idTp: idTp,
                                operType: '01',
                                manualCheck:true    //通过证件照片客服审核直通车入口上传时，该参数送true。
                            }),
                            success: function (result) {
                                ...
                                if (result.returnCode == 0) {
                                    ...
                                    window.location.href = "./index.html" + location.search;  //提交成功跳转到个人信息页面
                                } else {
