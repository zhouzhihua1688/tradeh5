...
// 20220705 mobile后台要求，wap页面所有请求都带有cookie值loginFrom:'W'
setCookie('loginFrom', 'W');
...
if(getCookie('channelCode') === 'airstar' || getCookie('channelCode') === 'zhengtong'){
...
// 20220712 10409 追踪码 设置url中的traceCode到cookie
var traceCode = getUrlParam('traceCode');
if(traceCode){
	setCookie('traceCode', traceCode);
}
...
if(document.domain.slice(-10) == '99fund.com'  ){//生产
	...
	sdk_urls= '//static.99fund.com/js/stat/sensorsdata-1.21.6/sensorsdata.min.js';
	heatmap_urls= "//static.99fund.com/js/stat/sensorsdata-1.21.6/heatmap.min.js";
}else{
    server_urls = 'http://10.50.18.1:8106/sa?project=default';
	sdk_urls= 'http://10.50.115.48/js/stat/sensorsdata-1.21.6/sensorsdata.min.js';
	heatmap_urls= "http://10.50.115.48/js/stat/sensorsdata-1.21.6/heatmap.min.js";
}

// 20220211 神策埋点引用部分异步改为同步，为了使用sensors.use('PageLeave') S
// (function(para) {
...
if(typeof(sensors) === 'undefined') {
    var outerScript = document.createElement('script'); outerScript.type = 'text/javascript'; outerScript.async = true;
    outerScript.src = sdk_urls;
    outerScript.onload = function(){
        console.log('outerScript.onload');
        sensors = window['sensorsDataAnalytic201505'];
        if(!sensors){
            return;
        }
        sensors.init({
            server_url: server_urls, //"数据接收地址",
            // is_track_single_page:true, // 单页面配置，默认开启，若页面中有锚点设计，需要将该配置删除，否则触发锚点会多触发 $pageview 事件
            app_js_bridge: true, // App打通H5
        ...
        sensors.quick('autoTrack');
        sensors.use('PageLeave'); //异步形式不能这样添加使用，必须改为同步形式
    };
    document.getElementsByTagName('head')[0].appendChild(outerScript);
}
// 20220211 神策埋点引用部分异步改为同步，为了使用sensors.use('PageLeave') E
//神策数据脚本 2018.12.20 add by sunchanghong | changed 20190506 
