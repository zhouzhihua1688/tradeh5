 ...
 */
 ...
 window.utils = {
    ...
    jumpLoginPage: function(){
        ...
        switch(channelCode){
            ...
            case 'airstar':   // 小米金融
                ...
                break;
            case 'zhengtong':   // 证通
                window.location.href = '/tradeh5/newWap/auth/login_zhengtong.html?channelCode=' + channelCode;
                break;
            case 'miniProgram':
                // TODO 20210205
    ...
    silentLogin: function(callback){
        ...
        if(code && (channelCode == 'airstar' || channelCode == 'zhengtong' || utils.isWeixin()) ){
                ...
                switch(result.returnCode){
                    case 0:
                        ...
                        if(result.body && result.body.custInfo){ // 小米金融使用，custInfo
                            ...
                            //20220509 神策记录用户id
                            try {
                                var custNo = result.body.custInfo.custNo;
                                if(custNo && sensors){
    ...
    authWap: function(){
        ...
        try {
            ...
            if(!isApp()){ //20211230 如果在app内报错，不跳转到登陆页面
                window.location.replace(referUrl);
            }
        }
    ...
    // 
    authZhengtong: function(){
        // appId: 'F2021011500001',  // 生产 zhengtong
        var appId = '';
        var channelCode = utils.getCookie('channelCode');
        if(utils.isProdEnv()){
            channelCode == 'zhengtong' && (appId = 'F2021011500001');
        } else {
            // 当前测试环境，appId可以被替换
            channelCode == 'zhengtong' && (appId = '200007');
        }
        ...
        if(code){
            ...
            console.log('没有code参数');
            // utils.getCode(appId);
            window.location.href = '/tradeh5/newWap/auth/login_zhengtong.html?referUrl=' + encodeURIComponent(utils.removeChannelCodeFromUrl(document.location.href));
        }
                ...
                switch(result.returnCode){
                    ...
                    case 1003:
                        ...
                        if(channelCode === 'airstar'){
                            ...
                            // window.location.href = '/tradeh5/newWap/auth/login_zhengtong.html?referUrl=' + encodeURIComponent(referUrl);
                            window.location.replace('/tradeh5/newWap/auth/login_zhengtong.html?referUrl=' + encodeURIComponent(referUrl));
                        } else {
                    ...
                    case 1004:
                        ...
                        break;
                    case 4103:
                        //20210823 4103，用户未关注官微，且未在交易流程时则引导其关注官微；用户未关注官微，且在交易流程时则跳转到注册登陆页面
                        ...
                        if(!nonTrade && (utils.getUrlParam('next_jump' || utils.getUrlParam('referUrl') ))){
    ...
    ajax: function (settings) {
        ...
        settings.success = function(result,status,xhr){
            ...
            else if (result.returnCode == 51000) {
                // 20211209 交易超时returnCode==51000处理，点击弹窗的确认按钮，跳转到交易查询页面
                utils.showTips({
                    content: result.returnMsg,
                    confirmText: '确定',
                    complete: function () {
                        if(isApp()){
                            window.location.href = 'htffundxjb://action?type=tq';
                        } else {
            ...
            // 20062为零元购客户，不弹窗报错
            else if(result.returnCode == 20062 && result.returnMsg == ''){
                return;
            }
            else {
    ...
    _tipsTemplate: function (params) {
        ...
        params.showCancel && template.find('._showTips_confirm')[params.cancelButtonFirst?'before':'after'](`<button class="_showTips_cancel"  style="color:#008ae9;width: 100%;padding: .3em 0;font-size: 0.75em;border: none;background-color: transparent;outline: none;border-radius: 0;line-height: inherit;border-left: solid 1px #eeeeee;">${params.nextText?params.nextText:'取消'}</button>`)
        if(params.cancelButtonFirst){
            template.find('._showTips_confirm').css('border-left', '1px solid #eeeeee');
            template.find('._showTips_cancel').css('border-left', 'none');
        }
    ...
    showTips: function (params) {
        ...
        if (_params.showCancel) {
            ...
            $('#_showTips_ ._showTips_confirm')[_params.cancelButtonFirst?'before':'after']('<button class="_showTips_cancel"  style="color:#008ae9;width: 100%;padding: .3em 0;font-size: 0.75em;border: none;background-color: transparent;outline: none;border-radius: 0;line-height: inherit;border-left: solid 1px #eeeeee;">取消</button></button>')
            if(params.cancelButtonFirst){
                $('#_showTips_ ._showTips_confirm').css('border-left', '1px solid #eeeeee');
                $('#_showTips_ ._showTips_cancel').css('border-left', 'none');
            }
        ...
        //     title: '信息', //标题
        //     content: '哈哈哈哈', //内容
        //     textAlign: 'left',  // content对齐方式，可以设置为左对齐，默认为center
