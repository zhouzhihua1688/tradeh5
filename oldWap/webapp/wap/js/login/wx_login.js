...
$(function(){
	if(HTF.isWeixin()){
		if(App.isNotEmpty(App.getUrlParam("unbind"))){
            ...
            var openId = App.isNotEmpty(App.getUrlParam("openId")) ? App.getUrlParam("openId") : App.getSession("__openId");
            var openUid = App.isNotEmpty(App.getUrlParam("openUid")) ? App.getUrlParam("openUid") : App.getSession("__openUid");
...
function login(){
	...
	//	                $(".auth-code-pic").attr("src", App.projectNm + "/login_auth_code?system=" + new Date().getTime());
		            ...
		            if(App.isNotEmpty(referUrl) && referUrl.indexOf("http") > -1){
