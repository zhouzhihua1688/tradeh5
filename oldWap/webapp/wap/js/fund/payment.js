...
function purchase() {
    ...
    if (App.isNotEmpty(purAmt)) {
        ...
        App.post(url, JSON.stringify(data), null, function (result) {
            ...
            if(fundSt == '1'){
                App.setSession(App.serialNo_forword_url, "../fund/fundSubscriptionSuccessfully.html");
            }else{
                App.setSession(App.serialNo_forword_url, "../fund/fundPurchaseSuccessfully.html");
            }
...
function queryCard(data) {
    ...
    var url = "/mobile-bff/v1/fund/queryBnkCardList?currencyType=" +currencyType + "&fundId=" + fundIdOut + "&tradeType=00" + "&r=" + (Math.random() * 10000).toFixed(0);
    App.get(url, null, function (result) {
        if (result.body != undefined && result.body != null) {
...
function queryFundPurchaseInfo() {
    ...
    App.get(url, null, function (result) {
        if (result.body != undefined && result.body != null) {
            // console.log(result);
            $("#purTip").html(result.body.remark.replace("size=14", ""));
