...
function formattingIncomes(incomes, fundTp, selectedTab){
    ...
    for (var i = 0; i < len; i++) {
        ...
        if (fundTp == '1') {
            ...
            if(selectedTab == 0){
                /*** 万份收益*/
                yArr.push(Number(incomes[i].incomeunit).toFixed(4));
            }else{
                /*** 七日年化*/
            ...
            if(selectedTab == 0){
                ...
                /*** 净值走势*/
                yArr.push(Number(incomes[i].nav).toFixed(4));
            }
        }
    }
    ...
    if(incomes == undefined || incomes == null) {
        ...
        App.get(url,null,function(result){
            if (result.body != undefined && result.body != null){
                ...
                if (result.body != undefined && result.body != null){
                    var incomes = result.body.fundYieldList;
        ...
        if (HTF.isNotBlank(ssoCookie)) {
        	...
        	if((loginChannel != "" && entryChannel != "" && loginChannel != entryChannel) || (loginChannel == "" && entryChannel != "" )){
	            ...
	            if(HTF.isNotBlank(code)) {
	            	...
	            	if (type == 'MIP'){
				        HTF.next_jump = prefix + "/mobileEC/wap/fund/fund_mip.html?fundId=" + App.getUrlParam("fundId");
				    }else if (type == 'BUY'){
				        HTF.next_jump = prefix + "/mobileEC/wap/fund/payment.html?fundId=" + App.getUrlParam("fundId");
				    }
...
//     // return;
// });
if(App.getUrlParam("f") == "xjb" || App.getUrlParam("fundId") == "000330" || App.getUrlParam("fundId") == "009588" || App.getUrlParam("fundId") == "009589"){
	$(".buy-div").hide();
	$(".view-div").show();
	$("#view_feeRate").hide();
	$(".bottom").css("bottom","2.5rem");
...
$.ajax({
    ...
    success: function (result) {
            ...
            if(result.returnCode == "1000"){//重定向到登录页面
                ...
                loginUrl = "../login/login.html?referUrl=" + encodeURIComponent(curUrl);
