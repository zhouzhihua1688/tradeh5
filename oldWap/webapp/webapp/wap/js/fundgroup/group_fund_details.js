	...
	if(HTF.isWeixin()){
        ...
        if (HTF.isNotBlank(ssoCookie)) {
			...
			if(is_set_pwd_show_tip_err_code == "2"){
		        $(".tip2").show();
		        $("#add_bind_card").attr("href", "../card/bindCardInputPassword_1.html?referUrl=" + document.URL);
		        return;
		    }else if (type == 'MIP'){
...
var option_1 = {
    ...
    tooltip : { show: true, trigger: 'axis',
        ...
        formatter: function(params){
            var arr = [];
arr.push('<div style="width: 100%;margin-left: -92%;font-size: 14px;">20'+params[0].axisValue.replace(/-/g,'.') + '  ')
    ...
    App.get(App.projectNm + "/adviser/query_fundgroup_detail_info?groupId=" + groupId, null, function(result){
        ...
        if(result.body != undefined && result.body != null){
            ...
            if(detailInfo != undefined && detailInfo != ""){
				...
				$("#fundgroupDesc").html('<div class="tupian"><img src="../images/fundgroup/jianyi.png" alt=""></div><span style="font-weight:bold">配置建议：</span>'+detailInfo.fundgroupDesc);
				 if(App.isEmpty(detailInfo.proPageUrl)){
				  ...
				  var newStr = detailInfo.createtimeStr.split('-');
                ...
                if(detailInfo.halfYearProfit < 0){
                  ...
                  } else if (Number(detailInfo.halfYearProfit) == 0){
                    $("#profit_year").html("--");
                }
                ...
                for(var i in groupDetails){
                    ...
                    for(var j in subFunds){
                        ...
                        if (fundRiskLevel != undefined && fundRiskLevel != null) {
                            ...
                            fundRiskDesc = fund.fundRiskLevelNm+"风险R" + fundRiskLevel + "";
                        }
