var Layout = {
    ...
    wap_line:"<div style='height: .4rem;'><!--<img src='../img/account/huixian.png'>--></div>",
    ...
    drawingLayoutNew:function (panel, layout, index, frame, isAddLine){
        if(layout != undefined && layout != null && frame != undefined && frame != null){
            ...
            var url = utils.applicationNm + layout.requestUrl;
            if (layout.requestMethod == 'GET'){
                utils.get(url, function () {
                ...
                utils.post(url, null, null, function (result) {
                    if (result.body != undefined && result.body != null) {
                        Layout.drawingLayoutContent(layout, index, frame, result.body.theme);
                    }
                });
    ...
    drawingFundRecommend:function (frame, theme) {
        ...
        if (theme != undefined && theme != null && theme.length > 0) {
            theme[0].object.forEach(function (item) {
                ...
                if (item.threshold) {
                    tag += "<h4>" + (item.threshold + (item.thresholdUnit ? item.thresholdUnit : "")) + "</h4>";
                }

                var panel = frame.data_frame;
                if(!item.recomelementval || item.recomelementval == "null"){
...
function formatTargetDateStr(formate,date) {
    var o = {
...
function queryDepositRecords(blankClass, frame, theme) {
    ...
    var startTime = formatTargetDateStr("yyyyMMdd", new Date(curYear, 0,1));
    var endTime = formatTargetDateStr("yyyyMMdd", new Date(Number(curYear)+1, 0, 0));
        ...
        if (returnCode == 0) {
            ...
            if (payTradeRecordList != undefined && payTradeRecordList != null) {
                ...
                payTradeRecordList.forEach(function (item) {
                    ...
                    if(item.apdt){
                        currentAckDtFormat = formatTargetDateStr("yyyy-MM-dd", strToDate(item.apdt));
                    }
                    var panel = frame.data_frame;
                    var amt = !item.ackAmt ? item.subAmt : item.ackAmt;
