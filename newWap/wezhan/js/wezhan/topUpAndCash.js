var forwardUrl = prefix + '/mobileEC/wap/wezhan/service.html';
$(".xjj_take_back").hide();
$(function () {
    ...
    $(".xjb_take_back").attr("href", "../account/cash.html?forwardUrl="+forwardUrl);
    $(".xjb_recharge").attr("href", "../account/topup.html?forwardUrl="+forwardUrl);
   // $(".xjj_take_back").attr("href", "../fundgroup/redeem.html?groupId=A0081&groupType=08");
    $(".xjj_recharge").attr("href", "../fundgroup/payment.html?groupId=A0081&groupType=08&forwardUrl="+forwardUrl);
    queryContractPay();
//    queryQuickPass();
    queryCashProfit();
...
$(".eyes").click(function () {
    if(flag){
        $(".eyes").css({"background": "url(../img/account/closeEyes.png) no-repeat no-repeat","background-size": "100% 100%"});
        $(".enshow").show();
        $(".isshow").hide();
        flag=false;
    }else{
        $(".eyes").css({"background": "url(../img/account/openEyes.png) no-repeat no-repeat", "background-size": "100% 100%"});
        $(".enshow").hide();
        $(".isshow").show();
        flag=true;
    }
...
function queryContractPay() {
    ...
    App.get(url, null, function(result){
        if (result.body != undefined && result.body != null){
            ...
            $(".func_list_1").append(getHtml('工资宝', '../account/autoTopUpCM.html?handTp=C','一次设置，工资定期转入，轻松赚取收益', list1));
            //还款专区
			/*
            var list2 = new Array();
...
function queryQuickPass() {
    ...
    App.get(url, null, function(result){
        if (result.body != undefined && result.body != null){
            ...
            $(".func_list_2").append(getHtml('汇闪付', 'htffundxjb://action?type=xp&subType=wb','微信支付绑定现金宝，理财消费两不误', list1));
        }
    });
}
            ...
            if(body != undefined && body != null){
                ...
                if(body.smacAsset != undefined && body.smacAsset != null){
                    ...
                    if(smacAsset.totalAmt > 0){
                        ...
                        $(".xjb_take_back").hide();
                        $(".xjb_remark").html('<span>0.1折申购基金，快取额度5万</span>');
                    }
                }
                ...
                if(body.cashPlusAsset != undefined && body.cashPlusAsset != null){
                    ...
                    if(cashPlusAsset.totalAmt > 0){
                        ...
                        $(".xjj_take_back").attr("href", "../fundgroup/redeem.html?groupId=A0081&groupType=08&forwardUrl="+forwardUrl);
                        ...
                        $(".xjj_recharge").attr("href","../fundgroup/group_fund_details_xjj.html?groupId=A0081&groupType=08&forwardUrl="+forwardUrl);
                        $(".xjj_take_back").hide();
...
function queryGroupFundDetail(type){
    App.get(App.projectNm + "/adviser/query_fundgroup_detail_info?groupId=A0081", null, function(result){
        if(result.body != undefined && result.body != null){
