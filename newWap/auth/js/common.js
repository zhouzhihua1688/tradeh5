...
var HTF = {
	...
	wxRedirectDispatch : 'http://wx.99fund.com/weixin/redirect_dispatch.html',
    ...
    getCodeBase: function () {
        ...
        var url = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=" + HTF.appId + "&redirect_uri="+ encodeURIComponent(HTF.wapRedirectDispatch+"?channelCode="+channelCode+"&next_jump="+HTF.removeCodeFromUrl(HTF.next_jump) ) +"&response_type=code&scope=snsapi_base&state=STATE";
    ...
    getCodeUserInfo: function () {
        ...
        var url = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=" + HTF.appId + "&redirect_uri="+ encodeURIComponent(HTF.wapRedirectDispatch+"?channelCode="+channelCode+"&next_jump="+HTF.removeCodeFromUrl(HTF.next_jump)+"&channelCode="+channelCode) +"&response_type=code&scope=snsapi_userinfo";
...
var App = {
    ...
    loginPage : "/mobileEC/wap/login/login.html",
    sessionStorage : window.sessionStorage,
...
$(function(){
    ...
    var loadingImagePath = "https://wx.99fund.com/mobileEC/wap"; 
    ...
    if(window.location.pathname.indexOf('wapWeb') == -1){
        $(document.body).append("<div id='over' class='over'></div>"
            +"<div id='layout' class='layout' style='display:none'><div class='loading-img'><img src='"+ loadingImagePath +"/images/hook/loadingHUD.png' /></div><div class='f12 f-white'>加载中..</div></div>");
    }
    if(App.isWeixin()){
		/*
        $(".top-bar").hide();
...
function replaceTxt2Html(val){
	return App.replaceTxt(val, "如何提升限额？", "<a href='https://wx.99fund.com/mobileEC/common/bankCardUpDesc.html'>如何提升限额？</a>");
}
...
$(".goto_login").click(function () {
    window.location.href = "../login/login.html?referUrl=" + document.URL;
});
...
function sessionExpireLogin() {/*console.log(11);return;*/
    ...
    if (HTF.isWeixin()) {
        ...
        var jumpUrl = App.loginPage + "?referUrl=" + document.URL;
        if(isAndroidApp()) {
    ...
    //根据访问域名判断环境修改跳转地址前缀
	var prefix = "https://wx.99fund.com";
