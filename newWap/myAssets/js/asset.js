...
function getMoreList(classifyCode, currencyType, fundSortType) {
        ...
        success: function (res) {
            ...
            if (res.returnCode === 0) {
                ...
                // if ((classifyCode == '02') || (classifyCode == '04')) {
                    ...
                    <p class="transaction-query" data-type="${classifyCode=='02'?'1':classifyCode=='03'?'3':classifyCode=='04'?'4':''}"><span>${result.onWayCount=='0'?'交易查询':result.onWayCount+'笔交易确认中'}</span><img src="./img/arrow-white.png" alt=""></p>
                </div>`
                // }
                html += '</div>'
                if (classifyCode == '02' && result.currencyType == '156') {
                ...
                if (groupList && groupList.length > 0) {
                    groupList.forEach(function (item, index) {
                        ...
                        item.assetList.forEach(function (childItem, childIndex) {
                         ...
                         <div class="content-t" data-type="${classifyCode}" data-id="${childItem.productId}" data-balanceSerialNo="${childItem.assetSerialNo}" data-api="${childItem.api}" data-item="${encodeURIComponent(JSON.stringify(childItem))}">
                             <div class="content-t-l">
                                 <p class="text1">${childItem.productName} <span>${childItem.productId}</span>`;
